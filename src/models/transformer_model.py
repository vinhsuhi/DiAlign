...
class GraphTransformerMatching(nn.Module):
    ...
    def forward(self, noisy_data, s_mask, t_mask, graph_s_data, graph_t_data):
        """
        dict_keys(['t_int', 't', 'beta_t', 'alpha_s_bar', 'alpha_t_bar', 'noise_align', 'mask_align'])
        what is the input?
        ...
        for i in range(self.num_layers):
            ...
            target_messages = noisy_data['noise_align'].float() @ x_padded_trg
            source_messages = noisy_data['noise_align'].float().transpose(1, 2) @ x_padded_src
